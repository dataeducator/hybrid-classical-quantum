...
The automated leakage audit confirmed no residual data leakage after preprocessing. All three checksper-feature AUC scan (threshold 0.95), feature-target correlation screening (threshold 0.90), and train/test distribution shift (KS test, $p < 0.001$)passed without flags.
...
Notably, the quantum-only model achieved AUC 0.7942 with very high recall (0.9837), indicating that the quantum pathway learned a near-constant positive prediction strategyan artifact of class imbalance rather than genuine discriminative power.
